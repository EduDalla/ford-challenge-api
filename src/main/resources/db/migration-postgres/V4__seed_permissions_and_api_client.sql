...
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'clientes:read');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'clientes:write', 'Permite criar e atualizar clientes.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'clientes:write');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'clientes:delete', 'Permite remover clientes.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'clientes:delete');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'interacoes:read', 'Permite consultar interacoes.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'interacoes:read');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'interacoes:write', 'Permite criar interacoes.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'interacoes:write');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'interacoes:delete', 'Permite remover interacoes.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'interacoes:delete');

INSERT INTO permissions (codigo, descricao, ativo, criado_em)
SELECT 'ml:predict', 'Permite chamar o endpoint de predicao ML.', TRUE, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE codigo = 'ml:predict');
...
SELECT
    ...
    CURRENT_TIMESTAMP,
    NULL
WHERE NOT EXISTS (SELECT 1 FROM api_clients WHERE client_id = 'python-ml-service');

INSERT INTO api_client_permissions (api_client_id, permission_id)
SELECT ac.id, p.id
FROM api_clients ac
...
WHERE ac.client_id = 'python-ml-service'
  AND NOT EXISTS (
      SELECT 1
      FROM api_client_permissions ap
